...
public class Main {
     ...
     * @return
     */
    public double helper(int poured, int query_row, int query_glass) {
     ...
     * 模拟：一次性倒香槟，满了就平分给下一层
     * 动态规划五步
     * dp数组下标含义:dp[i][j]表示第i行第j个玻璃杯所盛放的比例
     * 状态转移方程:对于大于1的dp[i][j]: 递推
     * dp[i+1][j] += (dp[i][j]-1)/2;
     * dp[i+1][j+1] += (dp[i][j]-1)/2;
     * 初始化：dp[0][0] = poured
     * 遍历顺序：外层 i 从 0~row-1; 内层 j 从0～i
     * 返回值：Math.min(1,dp[row][query_glass])
     *
     * @param poured      倾倒香槟总杯数
     * @param query_row   杯子的位置数
     * @param query_glass 行数
     * @return
     */
    public double solution(int poured, int query_row, int query_glass) {
        int n = query_row;
        double[][] dp = new double[n + 1][n + 1];
        dp[0][0] = poured;
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j <= i; j++) {
                double add = (dp[i][j] - 1) / 2;
                // 还有剩余
                if (add > 0) {
                    dp[i + 1][j] += add;
                    dp[i + 1][j + 1] += add;
